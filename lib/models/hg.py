...
class Hourglass(nn.Module):
  ...
  def forward(self, x):
    ...
    up2 = F.interpolate(low3, scale_factor=2) #self.up2(low3)
    
    return up1 + up2

class HourglassNet(nn.Module):
  def __init__(self, nInChan=3, nStack=2, nModules=2, nFeats=256, numOutput=28):
    super(HourglassNet, self).__init__()
    self.nStack = nStack
    self.nModules = nModules
    self.nFeats = nFeats
    self.numOutput = numOutput
    self.conv1_ = nn.Conv2d(nInChan, 64, bias = True, kernel_size = 7, stride = 2, padding = 3)
  ...
  def forward(self, x):
    ...
    return out[-1] # Just need final output
